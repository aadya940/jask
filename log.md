...
- **`ScalarMul`'s multiplier can't be a jit-traced argument.** Only works as a Python literal closed over in the function body (the pattern the whole test suite uses) - `make_op` bakes kwargs as compile-time constants, so a scalar passed as an actual argument to a jitted function (e.g. a learning rate) fails. Structural, not a one-line fix - would need `make_op` to support treating a kwarg as a real array-typed primitive input.
