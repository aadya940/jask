...
## Page-size derivation replaced with a correctly-derived, per-op-type formula

Found via a real (updated, re-run) jask-vs-dask benchmark crash: `page_size` used to come from a fixed, arbitrary `pages_per_group=3` divisor applied uniformly to every op, and `set_memory_budget`'s stated budget was never checked against actual system conditions at all (a `psutil.available`-based `Policy.host_capacity` existed but was dead code, never consulted). Both fixed, on the `page-size-derivation` branch:

- **`_peak_blocks(num_inputs, phase, pipelined)`** (`base_page.py`) replaces the constant, using the actual traced peak-resident-block counts: `num_inputs + 2` (forward), `2*num_inputs + 2` (backward), doubled if pipelined. `derive_page_shape` now takes `num_inputs`/`phase`/`pipelined` explicitly instead of reading a precomputed `policy.page_size` - computed once per op call (`HiOpGenerated`/`HiOpBackward`'s own `expand()`, which know their own `num_inputs`), not per-array independently, fixing a real latent risk where different arrays in the same op call could silently derive inconsistent block sizes.
- **`set_memory_budget` now clamps once, at call time**: `effective = min(user_value, 0.8 * available_at_that_moment)`. Deliberately a one-time snapshot, not re-checked per op call or continuously - static page size once set, matching how Dask's own chunk size behaves (fixed at creation, never varies based on live conditions) rather than making block size a moving target during execution.
- Removed now-fully-dead code: `Policy.page_size`/`pages_per_group`/`resident_pages`, `mark_loaded`/`mark_evicted`, `device_capacity`/`host_capacity`/`working_capacity`, and `OOCAlgorithm`'s now-unused `policy` parameter.
- 12 new tests (`test_page_size.py`): formula correctness across `(num_inputs, phase, pipelined)`, backward correctly getting smaller blocks than forward, determinism, and both clamp behaviors (leaves a conservative budget alone, clamps an optimistic one).
- Known, accepted limitation carried over explicitly: the clamp only protects against a stale/wrong budget *at the moment `set_memory_budget` is called* - not against available memory dropping later in a long-running session, and its effectiveness depends on the user calling `set_memory_budget` at a point where `available` actually reflects their application's real conditions.
- Not yet implemented (prototyped and measured in `/tmp` only, ~1.3x speedup, correctness confirmed, peak memory increase stays within the already-reserved headroom): pipelining the next block's disk read against the current block's `device_put`+compute via a single-worker `ThreadPoolExecutor`. The `pipelined` flag already exists in the formula above for when this lands for real.
